...
class BinarySearchTree {
  ...
  traverseDFSInOrder(node, list) {
    // Push the very left node first to the list
    if (node.left) {
      this.traverseDFSInOrder(node.left, list);
    }
    list.push(node.value);
    if (node.right) {
      this.traverseDFSInOrder(node.right, list);
    }

    return list;
  }

  traverseDFSPreOrder(node, list) {
    list.push(node.value);
    // Push the very left node first to the list
    if (node.left) {
      this.traverseDFSPreOrder(node.left, list);
    }
    if (node.right) {
      this.traverseDFSPreOrder(node.right, list);
    }

    return list;
  }

  traverseDFSPostOrder(node, list) {
    // Push the very left node first to the list
    if (node.left) {
      this.traverseDFSPostOrder(node.left, list);
    }
    if (node.right) {
      this.traverseDFSPostOrder(node.right, list);
    }
    list.push(node.value);
